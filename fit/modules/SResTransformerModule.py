...
class SResTransformerModule(LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = Adam(self.sres.parameters(), lr=self.hparams.lr, weight_decay=self.hparams.weight_decay)
    ...
    #     fc, (mag_min, mag_max) = batch
    #     model_path = self.saved_model_path or self.trainer.checkpoint_callback.last_model_path
    ...
    # #     pred[:,:,self.input_seq_length:] = torch.tensor(0.0,device = self.device)
    ...
    # #         for i in range(n):
    # #             x_hat.append(fcs[:, i])
    # #             yi, state = self.sres(x_hat[-1], i=i, state=state)
    # #             y_hat.append(yi)

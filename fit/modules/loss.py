...
def _fc_prod_loss(pred_fc, target_fc, amp_min, amp_max,w_amp=1,w_phi = 1):
    ...
    return torch.mean(w_phi*w_amp*amp_loss * phi_loss), torch.mean(amp_loss), torch.mean(phi_loss), torch.mean(w_phi*phi_loss)
...
def _fc_sum_loss(pred_fc, target_fc, amp_min, amp_max,w_amp=1,w_phi = 1):
    ...
    return torch.mean(w_amp * amp_loss + w_phi * phi_loss), torch.mean(amp_loss), torch.mean(phi_loss), torch.mean(w_phi*phi_loss)

def _fc_sum_loss_log_weighted(pred_fc, target_fc, amp_min, amp_max,w_amp=1,w_phi = 1):
    pred_amp = denormalize_amp(pred_fc[..., 0], amp_min=amp_min, amp_max=amp_max)
    target_amp = denormalize_amp(target_fc[..., 0], amp_min=amp_min, amp_max=amp_max)
    ...
    amp_loss = torch.pow(pred_amp - target_amp, 2)
    phi_loss = 1 - torch.cos(pred_phi - target_phi)
    w_phi = 10*max(torch.round(torch.pow(10,torch.round(torch.log10(torch.mean(amp_loss))))),1)
    return torch.mean(w_amp * amp_loss + w_phi * phi_loss), torch.mean(amp_loss), torch.mean(phi_loss), torch.mean(w_phi*phi_loss)
...
def _fc_L2_loss_sum_abs(pred_fc, target_fc, amp_min, amp_max,w_amp=1,w_phi = 1):
    ...
    return torch.sum(w_amp * amp_loss + w_phi * phi_loss), torch.sum(amp_loss), torch.sum(phi_loss), torch.mean(w_phi*phi_loss)
...
def _fc_sum_loss_w_dist(pred_fc, target_fc, amp_min, amp_max,w_amp=1,w_phi = 1):
    pred_amp = denormalize_amp(pred_fc[..., 0], amp_min=amp_min, amp_max=amp_max)
    target_amp = denormalize_amp(target_fc[..., 0], amp_min=amp_min, amp_max=amp_max)
    ...
    amp_loss = torch.pow(pred_amp - target_amp, 2)
    phi_loss = 1 - torch.cos(pred_phi - target_phi)
    phi_dist = torch.pow(pred_phi - target_phi, 2)
    return torch.mean(w_amp * amp_loss + w_phi * phi_loss + torch.sqrt(amp_loss+phi_dist)), torch.mean(amp_loss), torch.mean(phi_loss), torch.mean(w_phi*phi_loss)

def _fc_prod_loss_w_dist(pred_fc, target_fc, amp_min, amp_max,w_amp=1,w_phi = 1):
    pred_amp = denormalize_amp(pred_fc[..., 0], amp_min=amp_min, amp_max=amp_max)
    target_amp = denormalize_amp(target_fc[..., 0], amp_min=amp_min, amp_max=amp_max)
    ...
    phi_loss = 2 - torch.cos(pred_phi - target_phi)
    phi_dist = torch.pow(pred_phi - target_phi, 2)
    return torch.mean(w_phi * w_amp * amp_loss * phi_loss + torch.sqrt(amp_loss-1+phi_dist)), torch.mean(amp_loss), torch.mean(phi_loss), torch.mean(w_phi*phi_loss)